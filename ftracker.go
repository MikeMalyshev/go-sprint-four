...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	mSpeed := meanSpeed(action, duration)
	// ((18 * СредняяСкоростьВКм/ч * 1.79) * ВесСпортсменаВКг / mInKM  * ВремяТренировкиВЧасах * minInH)
	return (runningCaloriesMeanSpeedMultiplier * mSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(mSpeed*kmhInMsec, 2.0)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	mSpeed := swimmingMeanSpeed(lengthPool, countPool, duration)

	// (СредняяСкоростьВКм/ч + 1.1) * 2 * ВесСпортсменаВКг * ВремяТренеровкиВЧасах
	return (mSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
